...
const getStripePromise = async () => {
  try {
    const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/payment/config`);
    const config = await response.json();
    return loadStripe(config.publishableKey);
  } catch (error) {
    console.error('Failed to fetch Stripe config:', error);
    // Fallback to hardcoded key for development
    return loadStripe('pk_test_51RywhJ6B0M3J7IfCL0CRov22xwCoXhzJVjqzdrYBw4j2Itb0yRv8WuQksxdQP2ZRDCoXoedtEK63wH6yqbHYHJM000QWownNVA');
  }
};
